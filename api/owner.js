...
router.post("/order", async (req, res) => {
    ...
    const con = await connection()
    try{
        const result = await con.query("INSERT INTO tb_orderItem (ownerID, clinicID, itemID, amount) VALUES (?, ?, ?, ?);",
        [input.ownerID, input.clinicID, input.itemID, input.amount])
        if (!result.length) throw new Error("Something went wrong")
...
router.get("/orderHistory", async (req, res) => {
    ...
    const con = await connection()
    try{
        const result = await con.query("SELECT ownerID, clinicID, itemID, amount, time, totalPrice FROM tb_orderItem WHERE ownerID IS NOT NULL ORDER BY time DESC;")
        if (!result.length) throw new Error("Something went wrong")
